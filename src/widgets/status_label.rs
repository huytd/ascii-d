use druid::{Color, FontDescriptor, FontFamily, Point, Rect, RenderContext, Size, TextLayout, Widget};
...
impl StatusLabel {
    ...
    fn update_label(&mut self, data: &ApplicationState) {
        ...
        self.text.set_font(FontDescriptor::new(FontFamily::MONOSPACE).with_size(16.0));
...
impl Widget<ApplicationState> for StatusLabel {
    ...
    fn paint(&mut self, ctx: &mut druid::PaintCtx, _data: &ApplicationState, env: &druid::Env) {
        ...
        let rect = Rect::new(20.0, size.height - 50.0, 20.0 + text_size.size.width + 20.0, size.height - 20.0).to_rounded_rect(5.0);
        ctx.fill(rect, &brush);
